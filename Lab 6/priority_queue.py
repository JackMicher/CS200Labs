...
class HeapPriorityQueue(PriorityQueueBase):
    ...
    def pushpop(self, k, v):
        if self.is_empty():          # If tree is empty, return (k, v) as if it were the only node in the tree
            return (k, v)
        if k <= self._data[0]._key:  # If key entered is less than the min key, return (k, v) as if it were put into the tree and became the root
            return (k, v)
        item = self._data[0]
    ...
    def replace(self, k, v):
        if self.is_empty():                # If tree is empty, return error as if trying to remove from an empty list
